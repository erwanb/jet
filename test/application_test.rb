  ...
  describe "Builder" do
    ...
    describe '#build_asset' do
      ...
      it 'writes app/stylesheets/application.css to build dir as "application.css"' do
        ...
        @application.build_asset(:stylesheet)
        assert expected_path.exist?
    ...
    describe '#build_all' do
      it 'build javascript, stylesheets, copy static dir to build dir and copy prototypes to build dir' do
        @application.expects(:build_asset).with(:javascript).once
        @application.expects(:build_asset).with(:stylesheet).once
        @application.expects(:copy_static_assets_to_build).once
        @application.build
      end
    end

    describe "error handling for sprockets assets" do
      it 'writes in application.js an exception messsage' do
        @application = Jet::Application.new(:root_path => fixtures_path.join('sprockets_exception').to_s)
        @application.build_asset(:javascript)
        IO.read(@application.build_path.join('application.js')).must_match(/ExecJS::RuntimeError/)
      end

      it 'writes in application.css an exception messsage' do
        @application = Jet::Application.new(:root_path => fixtures_path.join('sprockets_exception').to_s)
        @application.build_asset(:stylesheet)
        IO.read(@application.build_path.join('application.css')).must_match(/Invalid CSS after/)
      end
    end
  end
  ...
  describe 'Watcher' do
    describe '#run_on_change' do
      ...
      it 'does not build javascript if there are no js files' do
        @application.expects(:build_asset).with(:javascript).never
      ...
      it 'does not build stylesheet if there are no css files' do
        @application.expects(:build_asset).with(:stylesheet).never
